...
{
        public static final String CHEST = "bankChest";
    public static final WeightedRandomChestContent[] bankChestContents = new WeightedRandomChestContent[]{
    ...
    private static final int HEIGHT = 10;
        ...
        protected ComponentVillageBank(ComponentVillageStartPiece startPiece, int type, Random random, StructureBoundingBox _boundingBox, int direction){
        ...
        if(canVillageGoDeeper(_boundingBox)){ 
                if(StructureComponent.findIntersecting(pieces, _boundingBox) == null){
                        return new ComponentVillageBank(startPiece, type, random, _boundingBox, direction);
                }
        }
                return null;
        }
    ...
    protected int getVillagerType(int par1){
        ...
        public static void registerTavernChest(){
                ChestGenHooks.getInfo(CHEST);
                   for(int i = 0; i < bankChestContents.length; i++){
                        ChestGenHooks.addItem(CHEST, bankChestContents[i]);
                }
        }
}